...
class MMOBomb:
	...
	def get_games_by_tag(self, tag: str):
		return get(f"{self.api}/games?tag={tag}", headers=self.headers).json()
	
	def sort_games(self, sort: str):
		return get(
			f"{self.api}/games?sort-by={sort}", headers=self.headers).json()
	...
	def get_mmo_giveaways(self):
		return get(f"{self.api}/giveaways", headers=self.headers).json()
	
	def get_latest_mmo_news(self):
		return get(f"{self.api}/latestnews", headers=self.headers).json()
